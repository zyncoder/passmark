"use client"

import * as React from "react"
import Link from "next/link"
import { createClient } from "@/utils/supabase/client"
import { useRouter } from "next/navigation"
import { Card, CardContent } from "@/components/ui/card"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"

const ACTIVITY_LABEL: Record<string, string> = {
  FORM_SUBMITTED: "Form submitted",
  FORM_APPROVED: "Form approved",
  FORM_REJECTED: "Form rejected",
  FORM_REOPENED: "Reopened for info",
  CREDENTIAL_MINTED: "Credential minted",
  CREDENTIAL_INVALIDATED: "Credential revoked",
  PRINT_STARTED: "Printing started",
  PRINT_COMPLETED: "Printed",
  QR_MAPPED: "QR mapped",
  SCAN_ALLOW: "Scan: allowed",
  SCAN_DENY: "Scan: denied",
  SCAN_ANOMALY: "Scan anomaly",
}

export default function AdminDashboardPage() {
  const router = useRouter()
  const supabase = createClient()
  const [stats, setStats] = React.useState({
    vendors: 0,
    total: 0,
    pending: 0,
    approved: 0,
    printed: 0,
    guards: 0,
  })
  const [recentApps, setRecentApps] = React.useState<any[]>([])
  const [activity, setActivity] = React.useState<any[]>([])
  const [loading, setLoading] = React.useState(true)

  React.useEffect(() => {
    async function load() {
      const [
        { count: vendors },
        { count: total },
        { count: pending },
        { count: approved },
        { count: printed },
        { count: guards },
      ] = await Promise.all([
        supabase.from("vendor_profiles").select("*", { count: "exact", head: true }),
        supabase.from("applications").select("*", { count: "exact", head: true }),
        supabase
          .from("applications")
          .select("*", { count: "exact", head: true })
          .in("status", ["SUBMITTED", "UNDER_REVIEW"]),
        supabase.from("applications").select("*", { count: "exact", head: true }).eq("status", "APPROVED"),
        supabase.from("credentials").select("*", { count: "exact", head: true }).in("print_status", ["PRINTED", "MAPPED"]),
        supabase.from("guards").select("*", { count: "exact", head: true }).eq("is_active", true),
      ])
      setStats({
        vendors: vendors || 0,
        total: total || 0,
        pending: pending || 0,
        approved: approved || 0,
        printed: printed || 0,
        guards: guards || 0,
      })

      const { data: apps } = await supabase
        .from("applications")
        .select(`*, vendor_profiles ( org_name )`)
        .neq("status", "DRAFT")
        .order("updated_at", { ascending: false })
        .limit(10)
      setRecentApps(apps || [])

      const { data: feed } = await supabase
        .from("activity_events")
        .select("*")
        .order("created_at", { ascending: false })
        .limit(20)
      setActivity(feed || [])

      setLoading(false)
    }
    load()
  }, [supabase])

  if (loading) return <div className="p-8">Loading dashboard...</div>

  const cards = [
    { label: "Vendors", value: stats.vendors },
    { label: "Forms", value: stats.total },
    { label: "Pending", value: stats.pending },
    { label: "Approved", value: stats.approved },
    { label: "Printed", value: stats.printed },
    { label: "Guards Active", value: stats.guards },
  ]

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-neutral-900">Admin Dashboard</h1>
        <p className="text-[14px] text-neutral-600 mt-1">System overview and recent activity.</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
        {cards.map((c, i) => (
          <Card key={i}>
            <CardContent className="p-6 flex flex-col justify-center h-full text-center">
              <h3 className="text-[12px] font-semibold text-neutral-600 mb-1 uppercase tracking-wider">{c.label}</h3>
              <p className="text-3xl font-bold text-neutral-900">{c.value}</p>
            </CardContent>
          </Card>
        ))}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          <div className="p-4 sm:p-6 border-b border-neutral-100 flex justify-between items-center">
            <h2 className="text-lg font-semibold">Recent Applications</h2>
            <Button variant="secondary" onClick={() => router.push("/admin/applications")}>
              View All
            </Button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-[14px] whitespace-nowrap">
              <thead className="bg-neutral-50 text-neutral-600 border-b border-neutral-100">
                <tr>
                  <th className="px-6 py-4 font-medium">Applicant</th>
                  <th className="px-6 py-4 font-medium">Organisation</th>
                  <th className="px-6 py-4 font-medium">Date</th>
                  <th className="px-6 py-4 font-medium">Status</th>
                  <th className="px-6 py-4 font-medium text-right">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-neutral-100">
                {recentApps.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-neutral-600">
                      No recent applications.
                    </td>
                  </tr>
                ) : (
                  recentApps.map((app) => (
                    <tr key={app.id} className="hover:bg-neutral-50/50">
                      <td className="px-6 py-4 font-medium text-neutral-900">
                        {app.first_name} {app.last_name}
                      </td>
                      <td className="px-6 py-4 text-neutral-600">
                        {app.vendor_profiles?.org_name}
                      </td>
                      <td className="px-6 py-4 text-neutral-600">
                        {new Date(app.updated_at).toLocaleDateString("en-GB")}
                      </td>
                      <td className="px-6 py-4"><Badge status={app.status as any} /></td>
                      <td className="px-6 py-4 text-right">
                        <Link href={`/admin/applications/${app.id}`} className="text-brand hover:text-brand-dark font-medium">
                          Review
                        </Link>
                      </td>
                    </tr>
                  ))
                )}
              </tbody>
            </table>
          </div>
        </Card>

        <Card>
          <div className="p-4 sm:p-6 border-b border-neutral-100">
            <h2 className="text-lg font-semibold">Activity Feed</h2>
            <p className="text-[13px] text-neutral-600 mt-1">Latest 20 system events.</p>
          </div>
          <div className="max-h-[460px] overflow-y-auto divide-y divide-neutral-100">
            {activity.length === 0 && (
              <p className="p-6 text-[14px] text-neutral-500">No activity yet.</p>
            )}
            {activity.map((e) => (
              <div key={e.id} className="px-5 py-3 text-[13px]">
                <div className="flex justify-between items-baseline">
                  <span className="font-medium text-neutral-900">
                    {ACTIVITY_LABEL[e.action] ?? e.action}
                  </span>
                  <span className="text-[11px] text-neutral-500">
                    {new Date(e.created_at).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}
                  </span>
                </div>
                {e.metadata?.applicant && (
                  <p className="text-neutral-600 mt-0.5">{e.metadata.applicant}</p>
                )}
                {e.metadata?.remarks && (
                  <p className="text-neutral-500 italic mt-0.5">"{e.metadata.remarks}"</p>
                )}
              </div>
            ))}
          </div>
        </Card>
      </div>
    </div>
  )
}
